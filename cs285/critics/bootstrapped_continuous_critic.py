...
class BootstrappedContinuousCritic(nn.Module, BaseCritic):
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        for t in range(self.num_grad_steps_per_target_update * self.num_target_updates):
            if t % self.num_grad_steps_per_target_update == 0:
                V_sp1 = self.forward(next_ob_no)
                V_target = reward_n + self.gamma * V_sp1 * (1 - terminal_n)
                V_target = V_target.detach()

            V_pred = self.forward(ob_no)
            loss = self.loss(V_pred, V_target)
    ...
    def multitask_update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        """
            Update the parameters of the critic.
        ...
        #       that its dimensions match the reward
        num_tasks = len(ob_no)
        ob_no = [ptu.from_numpy(ob_no[T]) for T in range(num_tasks)]
        ac_na = [ptu.from_numpy(ac_na[T]) for T in range(num_tasks)]
        next_ob_no = [ptu.from_numpy(next_ob_no[T]) for T in range(num_tasks)]
        reward_n = [ptu.from_numpy(reward_n[T]) for T in range(num_tasks)]
        terminal_n = [ptu.from_numpy(terminal_n[T]) for T in range(num_tasks)]

        # compute loss for each task
        loss_tasks = [0 for _ in range(num_tasks)]
        # compute target value for each task   
        V_target_tasks = [0 for _ in range(num_tasks)]

        for t in range(self.num_grad_steps_per_target_update * self.num_target_updates):
            if t % self.num_grad_steps_per_target_update == 0:
                for T in range(num_tasks):
                    V_sp1 = self.forward(next_ob_no[T])
                    V_target_tasks[T] = reward_n[T] + self.gamma * V_sp1 * (1 - terminal_n[T])

            for T in range(num_tasks):
                V_pred = self.forward(ob_no[T])
