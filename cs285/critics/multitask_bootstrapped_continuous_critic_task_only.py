...
class MultitaskBootstrappedContinuousCriticTaskOnly(nn.Module, BaseCritic):
    """
        Notes on notation:
    ...
    def __init__(self, hparams):
        ...
        # critic network 
        self.critic_task_head = [ptu.build_mlp(
            self.ob_dim, 
            1, 
            n_layers=self.n_layers, 
            size=self.size) for T in range(self.num_tasks)]
        ...
        for T in range(self.num_tasks):
            ...
            self.task_specific_optimizer[T] = optim.Adam(self.critic_task_head[T].parameters(), self.learning_rate)

    def forward(self, obs):
        task_specific_network_output = []
        for T in range(self.num_tasks):
            task_specific_network_output.append(self.critic_task_head[T](obs).squeeze(1))
        return task_specific_network_output

    def forward_np(self, obs):
        obs = [ptu.from_numpy(obs[T]) for T in range(self.num_tasks)]
    ...
    def forward_task(self, T, obs):
        return self.critic_task_head[T](obs).squeeze(1)
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        ob_no = [ptu.from_numpy(ob_no[T]) for T in range(self.num_tasks)]
        ac_na = [ptu.from_numpy(ac_na[T]) for T in range(self.num_tasks)]
        next_ob_no = [ptu.from_numpy(next_ob_no[T]) for T in range(self.num_tasks)]
        reward_n = [ptu.from_numpy(reward_n[T]) for T in range(self.num_tasks)]
        terminal_n = [ptu.from_numpy(terminal_n[T]) for T in range(self.num_tasks)]

        # compute loss for each task
        loss_tasks = [0 for _ in range(self.num_tasks)]
        # compute target value for each task   
        V_target_tasks = [0 for _ in range(self.num_tasks)]
