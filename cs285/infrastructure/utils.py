...
def sample_trajectory(env, policy, max_path_length, render=False, render_mode=('rgb_array')):
    ...
    while True:
        ...
        ac = ac[0] # dimension change for multitask
        acs.append(ac)
        ob, rew, done, _ = env.step(ac)
        # add the observation after taking a step to next_obs
        next_obs.append(ob)
...
def multitask_sample_trajectory(T, env, policy, max_path_length, render=False, render_mode=('rgb_array')):
    ob = env.reset()
    obs, acs, rewards, next_obs, terminals, image_obs = [], [], [], [], [], []
    steps = 0
    while True:
        ...
        obs.append(ob)
        # task specific actor
        if (isinstance(policy, MultitaskMLPPolicyAC) or isinstance(policy, ContinualMLPPolicyAC)):
            ac = policy.get_action(T, ob)
        # sharing actor
        if isinstance(policy, MLPPolicyAC):
            ac = policy.get_action(ob)
        ac = ac[0]
        acs.append(ac)
        ob, rew, done, _ = env.step(ac)
...
def multitask_sample_n_trajectories(T, env, policy, ntraj, max_path_length, render=False, render_mode=('rgb_array')):
    """
        Collect ntraj rollouts using policy
    """
    paths = []
    for i in range(ntraj):
        # collect rollout
        path = multitask_sample_trajectory(env, policy, max_path_length, render)
        paths.append(path)
    return paths

def multitask_sample_trajectories(T, env, policy, min_timesteps_per_batch, max_path_length, render=False, render_mode=('rgb_array')):
    """
        Collect rollouts using policy
        until we have collected min_timesteps_per_batch steps
    """
    ...
    while timesteps_this_batch < min_timesteps_per_batch:
        ...
        #collect rollout
        path = multitask_sample_trajectory(T, env, policy, max_path_length, render)
        paths.append(path)
