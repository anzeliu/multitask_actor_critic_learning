...
from cs285.critics.continual_bootstrapped_continuous_critic import \
    ContinualBootstrappedContinuousCritic
...
from cs285.policies.continual_MLP_policy import ContinualMLPPolicyAC
from cs285.policies.multitask_MLP_policy import MultitaskMLPPolicyAC
from cs285.policies.MLP_policy import MLPPolicyAC
from .base_agent import BaseAgent


class ContinualACAgent(BaseAgent):
    def __init__(self, env, new_env, agent_params):
        super(ContinualACAgent, self).__init__()
        ...
        # number of new tasks
        self.num_new_tasks = self.agent_params['num_new_tasks']
        ...
        self.actor = ContinualMLPPolicyAC(
            self.agent_params['ac_dim'],
            self.agent_params['ob_dim'],
            self.agent_params['n_layers'],
            self.agent_params['size'],
            self.agent_params['num_tasks'],
            self.agent_params['num_new_tasks'],
            self.agent_params['discrete'],
            self.agent_params['learning_rate'],
        )

        # Architecture 3 - shared + task specific critic
        self.critic_tasks = ContinualBootstrappedContinuousCritic(self.agent_params)
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        loss_tasks = [OrderedDict() for _ in range(self.total_tasks)]
        ...
        for _ in range(self.agent_params['num_actor_updates_per_agent_update']):
            ...
            if isinstance(self.actor, MultitaskMLPPolicyAC) or isinstance(self.actor, ContinualMLPPolicyAC):
    ...
    def estimate_advantage_tasks(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        adv_n_tasks = [0 for _ in range(self.total_tasks)]
    ...
    def add_new_task(self):
        ...
        self.critic_tasks.add_new_task()
        self.actor.add_new_task()
