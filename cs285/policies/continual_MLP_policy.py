...
class ContinualMLPPolicy(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
...
class ContinualMLPPolicyAC(ContinualMLPPolicy):
    def update(self, observations_tasks, actions_tasks, adv_n_tasks=None):
        # TODO: update the policy and return the loss
        observations_tasks = [ptu.from_numpy(observations_tasks[T]) for T in range(self.total_tasks)]
        actions_tasks = [ptu.from_numpy(actions_tasks[T]) for T in range(self.total_tasks)]
        adv_n_tasks = [ptu.from_numpy(adv_n_tasks[T])for T in range(self.total_tasks)]
    ...
    def add_new_task(self):
        ...
        if self.discrete:
            _, logits_na_tasks_list = ptu.build_multitask_network(
                input_size=self.ob_dim, 
                output_size = self.ac_dim,
                n_layers=self.n_layers,
                size=self.size, 
            ...
            for logits in logits_na_tasks_list:
                self.logits_na_tasks.append(logits)
            ...
            _, mean_net_tasks_list = ptu.build_multitask_network(
                input_size=self.ob_dim, 
                output_size = self.ac_dim,
                n_layers=self.n_layers,
                size=self.size, 
            ...
            for mean_net in mean_net_tasks_list:
                self.mean_net_tasks.append(mean_net)
