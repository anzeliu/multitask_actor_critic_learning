...
def main():
    ...
    if args.transfer_learning == True:
        ...
        params_targetA = params
        params_targetA['env_name'] = 'HalfCheetah-v4'
        params_targetA['exp_name'] = 'sac_HalfCheetah_v4'

        data_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), '../../data')
        if not (os.path.exists(data_path)):
            os.makedirs(data_path)
        logdir = params_targetA['exp_name'] + '_' + params_targetA['env_name'] + '_' + time.strftime("%d-%m-%Y_%H-%M-%S")
        logdir = os.path.join(data_path, logdir)
        params_targetA['logdir'] = logdir
        if not(os.path.exists(logdir)):
            os.makedirs(logdir)

        print("\n\n\nLOGGING TO: ", logdir, "\n\n\n")

        trainer_target = SAC_Trainer(params_targetA)
        trainer_target.rl_trainer.agent.actor = trainer.rl_trainer.agent.actor
        trainer_target.run_training_loop()
